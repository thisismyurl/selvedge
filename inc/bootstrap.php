 ...
 *
 * This is the ONE place a theme's identity lives. Change the `selvedge_` function
 * prefix and the SELVEDGE_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `selvedge_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Selvedge;`, on the reasoning that callbacks
 ...
 * rewrites `selvedge_` → `<slug>_`, `SELVEDGE_` → `<SLUG>_`, and `Selvedge_` →
 ...
 * stays a string LITERAL ('selvedge'), never the SELVEDGE_SLUG constant. `wp i18n
 ...
 */
define( 'SELVEDGE_VERSION', '1.6265.1630' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'SELVEDGE_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'SELVEDGE_URI', get_template_directory_uri() );
