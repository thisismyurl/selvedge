...
function selvedge_register_bindings(): void {
	...
	register_block_bindings_source(
		SELVEDGE_SLUG . '/footer-credit',
		array(
			'label'              => esc_html__( 'Footer credit', 'selvedge' ),
			'get_value_callback' => 'selvedge_get_footer_credit_value',
			'uses_context'       => array(),
		)
	);
}
 ...
 */
function selvedge_get_footer_credit_value(): string {
	$theme = wp_get_theme();
	$name  = $theme->get( 'Name' );
	...
	$text = sprintf(
		...
		esc_html__( 'Built with %s.', 'selvedge' ),
		$credit
	);
	 ...
	 */
	$text = (string) apply_filters( SELVEDGE_SLUG . '/footer_credit_text', $text ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
