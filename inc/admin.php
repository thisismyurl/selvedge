 ...
 */
const WELCOME_FLAG = 'selvedge_welcome_notice'; // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
const DISMISS_ACTION = 'selvedge_dismiss_welcome'; // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
const GET_STARTED_SLUG = 'selvedge-get-started';
...
function register_get_started_page(): void {
	add_theme_page(
		...
		sprintf( esc_html__( '%s: Get started', 'selvedge' ), get_theme_name() ),
		get_theme_name(),
...
function get_started_content(): array {
	...
	$default = array(
		'lead'       => sprintf(
			/* translators: %s: theme name. */
			__( '%s is a free, full-site-editing theme built to get out of the way of your content. Here is how to make it yours.', 'selvedge' ),
			$theme
		),
		'steps'      => array(
			array(
				'title' => __( 'Choose what people land on.', 'selvedge' ),
				'body'  => __( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'selvedge' ),
			),
			array(
				'title' => __( 'Give people a way around.', 'selvedge' ),
				'body'  => __( 'A site without a menu is a room without doors. Open the Site Editor, edit the header, and assign your menu to Primary Navigation.', 'selvedge' ),
			),
			array(
				'title' => __( 'Start from a pattern, not a blank page.', 'selvedge' ),
				'body'  => __( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'selvedge' ),
			),
			array(
				'title' => __( 'Make it sound like you.', 'selvedge' ),
				'body'  => __( 'In the Site Editor, open Styles to change colours and typefaces. Nothing you do there can break the theme — experiment freely.', 'selvedge' ),
			),
		),
		'optimize'   => array(
			__( "This theme is already fast by design: zero front-end JavaScript, self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'selvedge' ),
			__( "It's built toward WCAG 2.2 AA — real focus outlines, a skip link, sensible heading order, and motion that respects a reduce-motion setting. That's the structural floor, not an audited certification. Keep your own copy and images to that bar and the whole site stays welcoming.", 'selvedge' ),
		),
		'credit'     => __( "There's a small credit in your footer. It's a thank-you, not a tax — remove it in two clicks in the Site Editor → Footer, or filter it out in code. No hard feelings either way.", 'selvedge' ),
		'developers' => array(
...
function render_get_started_page(): void {
	...
	<div class="wrap selvedge-get-started">
		...
		<?php if ( ! empty( $content['credit'] ) ) : ?>
			<h2><?php esc_html_e( 'The footer credit', 'selvedge' ); ?></h2>
