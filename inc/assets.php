 ...
 *
 * Portable: every handle derives from the SELVEDGE_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the SELVEDGE_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function selvedge_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'selvedge_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function selvedge_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( SELVEDGE_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( SELVEDGE_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'selvedge_preload_fonts', 1 );
